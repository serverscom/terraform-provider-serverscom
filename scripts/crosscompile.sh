...
for PLATFORM in $PLATFORMS; do
  GOOS=${PLATFORM%/*}
  GOARCH=${PLATFORM#*/}
  BIN_FILENAME="terraform.d/plugins/${GOOS}_${GOARCH}/${OUTPUT}"
...
if [[ $PLATFORMS_ARM == *"linux"* ]]; then
  BIN_FILENAME="terraform.d/plugins/linux_arm64/${OUTPUT}"
  CMD="GOOS=linux GOARCH=arm64 go build -o ${BIN_FILENAME} $@"
  echo "${CMD}"
  eval $CMD || FAILURES="${FAILURES} ${PLATFORM}"
fi
for GOOS in $PLATFORMS_ARM; do
  ...
  for GOARM in 7 6 5; do
    BIN_FILENAME="terraform.d/plugins/${GOOS}_${GOARCH}${GOARM}/${OUTPUT}"
